...
function RecipeDetail({ recipe, onBack }) {
  ...
  // ADD YOUR EDAMAM CREDENTIALS HERE
  const APP_ID = '51b7e5c0';
  const APP_KEY = '4e20b9d61b8858f224f9ffcb4421bbdc';
  ...
  const fetchRecipeFromAPI = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `https://api.edamam.com/api/recipes/v2?type=public&q=${encodeURIComponent(ingredients)}&app_id=${APP_ID}&app_key=${APP_KEY}`
      );
      const data = await response.json();
    ...
    <div className="recipe-detail">
      ...
      <div className="recipe-header">
        <img 
          ...
          alt={mealDetails.label}
          className="recipe-image"
        />
        <div className="recipe-info">
          <h2>{mealDetails.label}</h2>
          <div className="recipe-meta">
            <span className="cuisine-tag">{mealDetails.cuisineType?.[0] || 'International'} cuisine</span>
            <span className="category-tag">{mealDetails.mealType?.[0] || 'Main dish'}</span>
          </div>
          <div className="match-info-detail">
            <p>✅ {recipe.matchPercentage}% ingredient match with what you have</p>
            <p>🍽️ Serves: {mealDetails.yield || 'N/A'}</p>
            <p>⏱️ {Math.round(mealDetails.totalTime) || 'N/A'} minutes</p>
          </div>
        </div>
      </div>
      ...
      <div className="recipe-content">
        <section className="ingredients-detail">
          <h3>📝 Ingredients:</h3>
          <ul className="ingredients-list">
            {mealDetails.ingredientLines.map((ing, i) => (
              <li key={i}>
          ...
          <div className="instructions">
            {mealDetails.url ? (
              <p className="instruction-step">
                <a 
                  href={mealDetails.url} 
                  target="_blank" 
                  rel="noopener noreferrer"
                  className="recipe-source-link"
                >
                  View full instructions on {mealDetails.source} →
                </a>
              </p>
            ) : (
        ...
        {mealDetails.url && (
          <section className="video-section">
            ...
            <a 
              ...
              className="video-link"
            >
              Visit {mealDetails.source} for full recipe →
            </a>
          </section>
        )}
      </div>
